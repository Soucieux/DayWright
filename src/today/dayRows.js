 ...
 * then the user's own tasks the plan doesn't schedule, marked as outside it.
 * @param {object} day - The day from the local service.
 * @returns {{rows: object[], fromPlan: boolean}} Rows sorted by start time.
 */
export function dayRows(day) {
  const fromPlan = Boolean(day.confirmedVariantId);
  const itemsById = new Map(day.dayItems.map((item) => [item.id, item]));
  ...
  const itemRows = day.dayItems.filter((item) => !scheduled.has(item.id))
    .map((item) => ({ ...item, kind: "item", source: item, outsidePlan: fromPlan }));
  return { rows: [...planRows, ...itemRows].sort((a, b) => a.start_time.localeCompare(b.start_time)), fromPlan };
}
